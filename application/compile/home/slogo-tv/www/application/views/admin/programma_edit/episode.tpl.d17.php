...
/* -- foreach end output */
		...
		$('input[name=Opslaan]').click(function(e){
			...
			request.open("POST", "http://text-tv.rtvslogo.nl/admin/programmering/<?php echo $this->scope["programme_data"]["programme_id"];?>/episode/<?php echo $this->scope["programme_data"]["episodes"]["0"]["episode_id"];?>");
			request.send(form);
...
<?php 
$_fh10_data = (isset($this->scope["programme_data"]["episodes"]) ? $this->scope["programme_data"]["episodes"]:null);
if ($this->isTraversable($_fh10_data) == true)
{
	foreach ($_fh10_data as $this->scope['episode'])
	{
/* -- foreach start output */
?>
	<div class="card">
		...
		<div class="body">
			...
			<label style="float:left; width:140px;">Tijden:</label>
				<div style="float: left;">
					<div id="times">
						<?php 
$_fh7_data = (isset($this->scope["episode"]["times"]) ? $this->scope["episode"]["times"]:null);
if ($this->isTraversable($_fh7_data) == true)
{
	foreach ($_fh7_data as $this->scope['time'])
	{
/* -- foreach start output */
...
/* -- foreach end output */
			...
			<label style="float:left; width:140px;">Periodes:</label>
				<div style="float: left;"><div id="periods">
					<?php 
$_fh8_data = (isset($this->scope["episode"]["periods"]) ? $this->scope["episode"]["periods"]:null);
if ($this->isTraversable($_fh8_data) == true)
{
	foreach ($_fh8_data as $this->scope['period'])
	{
/* -- foreach start output */
?>
...
/* -- foreach end output */
			...
			<label style="float:left; width:140px;">Sources:</label>
				<div style="float: left;"><ul id="sources">
					<?php 
$_fh9_data = (isset($this->scope["episode"]["sources"]) ? $this->scope["episode"]["sources"]:null);
if ($this->isTraversable($_fh9_data) == true)
{
	foreach ($_fh9_data as $this->scope['source'])
	{
/* -- foreach start output */
?>
						<?php if (! empty($this->scope["source"]["source_id"])) {
?>
							<li id="<?php echo $this->scope["source"]["link_id"];?>" order="vid_old_<?php echo $this->scope["source"]["link_id"];?>" class="source">
								<a href="#" class="remove_source" id="<?php echo $this->scope["source"]["link_id"];?>"><img style="width:10px;" src="/resources/cross-icon.png"></a>
								<?php echo form_dropdown('sources['.(isset($this->scope["source"]["link_id"]) ? $this->scope["source"]["link_id"]:null).'][source]', (isset($this->scope["sourcelist"]) ? $this->scope["sourcelist"] : null), (isset($this->scope["source"]["source_id"]) ? $this->scope["source"]["source_id"]:null));?>

								<?php echo form_hidden('sources['.(isset($this->scope["source"]["link_id"]) ? $this->scope["source"]["link_id"]:null).'][type]', 'source');?>
...
?>
							<li id="<?php echo $this->scope["source"]["link_id"];?>" order="html_old_<?php echo $this->scope["source"]["link_id"];?>" class="html">
								<a href="#" class="remove_html" id="<?php echo $this->scope["source"]["link_id"];?>"><img style="width:17px;" src="/resources/cross-icon.png"></a>
								<a href="#" class="edit_html" id="<?php echo $this->scope["source"]["link_id"];?>" data-title="Template bewerken"><img style="width:17px;" src="/resources/edit-circled.svg"></a>
								<?php echo form_dropdown('html_template', (isset($this->scope["templatelist"]) ? $this->scope["templatelist"] : null), (isset($this->scope["source"]["html_id"]) ? $this->scope["source"]["html_id"]:null), 'disabled');?>
...
<button type="button"><a href="<?php echo site_url();?>admin/programmering/duplicate?id=<?php echo $this->scope["programme_data"]["episodes"]["0"]["episode_id"];?>&programme=<?php echo $this->scope["programme_data"]["programme_id"];?>">Dupliceren</a></button>
<button type="button"><a href="<?php echo site_url();?>admin/programmering/delete?type=episode&id=<?php echo $this->scope["programme_data"]["episodes"]["0"]["episode_id"];?>&redirect=admin/programmering/edit/<?php echo $this->scope["programme_data"]["programme_id"];?>">Verwijderen</a></button>
<?php echo form_close();?>
