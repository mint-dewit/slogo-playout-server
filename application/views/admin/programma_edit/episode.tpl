...
<script>
	$(document).ready(function(){
		...
		$('input[name=Opslaan]').click(function(e){
			...
			request.open("POST", "http://text-tv.rtvslogo.nl/admin/programmering/{$programme_data.programme_id}/episode/{$programme_data.episodes.0.episode_id}");
			request.send(form);
...
<div class="title">{$programme_data.Naam} >> Afleveringen</div>
<div class="cards-wrapper">
{foreach $programme_data.episodes episode}
	<div class="card">
		<div class="top-bar">
			<span>#</span>
			<input type="number" name="number" value="{$episode.episode_number}" id="input_number" placeholder="?">
			{form_input('title','$episode.episode_title','id="input_title" placeholder="Naam..."')}
		</div>
		<div class="body">
			...
			<label style="float:left; width:140px;">Tijden:</label>
				<div style="float: left;">
					<div id="times">
						{foreach $episode.times time}<span id="{$time.id}" class="time">
							<a href="#" class="remove_time" id="{$time.id}"><img style="width:10px;" src="/resources/cross-icon.png"></a>{form_input('times[$time.id]','$time.time','class="hidden_input"')}<br />
						</span>{/foreach}
					</div>
			...
			<label style="float:left; width:140px;">Periodes:</label>
				<div style="float: left;"><div id="periods">
					{foreach $episode.periods period}
						<span class="period" id="{$period.id}">
							<a href="#" class="remove_period" id="{$period.id}"><img style="width:10px;" src="/resources/cross-icon.png"></a>
							{form_input('periods[$period.id][start]','$period.period_start','class="hidden_input"')} - 
							{form_input('periods[$period.id][end]','$period.period_end','class="hidden_input"')}
							<br />
						</span>
					{/foreach}</div>
			...
			<label style="float:left; width:140px;">Sources:</label>
				<div style="float: left;"><ul id="sources">
					{foreach $episode.sources source}
						{if !empty($source.source_id)}
							<li id="{$source.link_id}" order="vid_old_{$source.link_id}" class="source">
								<a href="#" class="remove_source" id="{$source.link_id}"><img style="width:10px;" src="/resources/cross-icon.png"></a>
								{form_dropdown('sources[$source.link_id][source]', $sourcelist, $source.source_id)}
								{form_hidden('sources[$source.link_id][type]', 'source')}
								<br />
							</li>
						{else}
							<li id="{$source.link_id}" order="html_old_{$source.link_id}" class="html">
								<a href="#" class="remove_html" id="{$source.link_id}"><img style="width:17px;" src="/resources/cross-icon.png"></a>
								<a href="#" class="edit_html" id="{$source.link_id}" data-title="Template bewerken"><img style="width:17px;" src="/resources/edit-circled.svg"></a>
								{form_dropdown('html_template', $templatelist, $source.html_id, 'disabled')}
							</li>
						{/if}
					{/foreach}
					</ul>
...
<button type="button"><a href="{site_url()}admin/programmering/duplicate?id={$programme_data.episodes.0.episode_id}&programme={$programme_data.programme_id}">Dupliceren</a></button>
<button type="button"><a href="{site_url()}admin/programmering/delete?type=episode&id={$programme_data.episodes.0.episode_id}&redirect=admin/programmering/edit/{$programme_data.programme_id}">Verwijderen</a></button>
{form_close()}
